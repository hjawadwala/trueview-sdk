...
ConnectApi.onrecvframeex = function (api_conn, frametype, data, datalen, channel, width, height, enc, fps, timestamp) {
  if(window.testRecordFlag){
    window.testVideAndAudioChunck.push(data)
  }
	//窗口已经被关闭
    ...
    for(let i = 0; i < MAX_SCREEN; i++){
      ...
      playerList[i].fillframe_v2(data, datalen, enc, timestamp, width, height, frametype);
    }
...
var doOnceCode =  function( uid, channel) {
	if (!executed) {
    var blob
    if( window.isHardDecode ){
      let decode_type = encode === 'H264'? 0 : 1
      const mp4Array = Module.convertNalus2Mp4(chunks, chunkTimeStamps, decode_type);
      blob = new Blob([mp4Array], {
       ...
       blob = new Blob(chunks, { 'type' : 'video/mp4' });
		...
		a.download = s.getFullYear().toString() + m + d + h + minute + secs + "_" + ((window.overTime - window.beginTime) / 1000).toString() + '.mp4';
...
var chunks = window.isHardDecode? new Uint8Array() : [];
var chunkTimeStamps = []
var encode = ''
...
//   console.log('data && frametype != 0',data && frametype != 0)
  if(window.overTime && data && frametype != 0){
  ...
  if(window.overTime){
	var tmpTs = new Date(ts_ms - new Date().getTimezoneOffset() *60 * 1000).valueOf();
  clearTimeout(downloadTimer)
  setDownloadTimer(tmpTs,api_conn.deviceid, channel)
  if(tmpTs+200>=overTime){
    clearTimeout(downloadTimer)
    doOnceCode(api_conn.deviceid, channel); 
  }
	var d =   window.downloadSpecifyTime? document.getElementById("downSpecifyTimeProcess") :document.getElementById("process");
	
	var p = (tmpTs - window.beginTime) / (window.overTime - window.beginTime) * 100;
	if(p>100)p=100;
	d.style.width = p + "%";
	var useTime = (Date.now() - window.useTimeStart) / 1000;
  ...
  if(!window.downloadSpecifyTime){
    document.getElementById("downSpeed").innerText = "Speed: " + speed + " kbps";
  }
 ...
 */
Player.ConnectDevice = function (devid, ip, user, pwd, winindex, port, connectType, channel, streamid, wss, cb, turnIp, turnPort) {
  console.log('ConnectDevice',{devid, ip, user, pwd, winindex, port, connectType, channel, streamid, wss, cb, turnIp, turnPort});
	//ID连接
	if (devid) {
		...
		if (bConnect) {
			...
			// ConnectApi.connectbyid(session, devid, useHttps, cb, turnIp, turnPort);
		}
		// else {
...
Player.StartPlayBack = function (id, ip, channel, begintime, endtime, type, winindex, isSound, tasktype) {
  ...
  playerList[winindex].playbackPause = false
	...
	if (session != null && session.logined) {
		...
		ConnectApi.replay_start(session, channel, begintime, endtime, filetype, tasktype)
		console.log('StartPlayBack中的replay_start参数',channel, begintime, endtime, filetype);
		if(tasktype==1) {
			window.initFlv = 0;
			return;
		};
